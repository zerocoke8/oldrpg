...
/** ★ 2 다. gates 는 필드가 **늘어난** 것이 아니라 **타입이 바뀌었다** —
 *  옛 클라이언트는 new Set(객체) 를 만들어 has("x,y") 가 영영 false 이고,
 *  크래시도 경고도 없이 출구 표시만 사라진다. 아래 world.flag 는 '추가' 라서
 *  안 올렸다 (불변식 3 이 덮는다). 이건 그 반대편이고, 그 조용함이 정확히
 *  이 상수가 막으라고 있는 것이다. 대가는 배포 순간 열려 있던 탭이 한 번
 *  새로고침을 요구받는 것이다. */
...
/** 나가는 길이 있는 칸과, 그 칸의 **어느 벽면**으로 나가는가.
 *
 *  ★ 방향은 격자에서 유도할 수 없다. 출구는 벽 자리에만 있지만, 운영 세계의
 *    출구 칸 열 곳이 **전부 벽 이웃이 셋**이라 유도는 언제나 3지선다로 끝난다.
 *    그래서 진짜로 실려야 한다 (test/world.ts 가 그 측정을 붙들고 있다).
 *  ★ 늘어나는 것은 '이미 공개된 칸의 네 면 중 어디' 뿐이다. 목적지 지역도
 *    requires 도 minRank 도 여전히 안 나간다 — 그 칸은 오늘도 실리고 있었다.
 *    가 보면 서버가 문장으로 답한다 (규칙 1). */
export interface GateView {
  /** "x,y" */
  at: string;
  /** 한 칸에 방향이 다른 출구가 둘일 수 있다 (server/engine/map.ts). */
  dirs: Dir[];
}
...
export interface RegionView {
  ...
  height: number;
  tiles: string[];
  /** 적이 배치된 칸들 ("x,y"). 미니맵이 표시를 찍는다.
   *  '지금 살아 있는가' 가 아니라 '여기서 나온다' 다 — 그쪽이 지도가 말할
   *  성질이고, 변하지 않으므로 스냅샷 한 번에 실려 나가면 끝이다.
   *  (스포일러가 아니다: 격자와 벽/바닥은 어차피 전부 실려 있었고, 씨앗도
   *   문장도 적의 정체도 여기 없다 — 프로토콜 불변식 2 는 그대로다.) */
  foes: string[];
  /** 나가는 길이 있는 칸들. 미니맵이 그 벽면에 표시를 찍는다. */
  gates: GateView[];
}
