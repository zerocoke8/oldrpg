...
export function rootItems(st: UiState): MenuItem[] {
  ...
  if (combat || st.room?.hasEnemy) {
    ...
    /* ★ 갈래가 '공격' 하나뿐이면 한 층을 접는다 — 교전 전이 정확히 그 상태다
       (스킬은 교전이 시작돼야 붙고 물러나기도 그때 생긴다). 들어가서 하나뿐인
       것을 다시 고르는 것은 목록이 아니라 확인 절차이고, 이 메뉴에 확인 절차가
       필요한 자리는 없다.

       ★ 이 저장소가 이미 한 번 내린 결정과 같은 모양이다: 남에게 걸 수 있는
         스킬도 allies 가 비면 목록을 열지 않고 곧장 자기에게 나간다(위 53줄).
         새 개념이 아니라 같은 규칙의 두 번째 적용이다.

       ★ 일반 규칙으로 두지 않는다. "하위가 하나면 접는다" 로 만들면 물약이
         하나 남은 가방을 여는 것이 곧 마시는 것이 되고, 방에 한 사람뿐이면
         건네기가 곧 주기가 된다 — 되돌릴 수 없는 것들이다.
       ★ 그래서 조건에 라벨을 적는다. length === 1 만 보면 "하나 남은 갈래가
         무엇이든" 접는 것이고, 이 접기의 안전성 논거("'싸우기' 라는 라벨이
         이미 공격을 뜻한다")가 코드 어디에도 안 남는다.
       ★ 공격이 안전한 이유는 '되돌릴 수 있어서' 가 아니다 — 물러나기는 자동
         공격만 끄고 위협은 남는다(combat.ts 의 stop). 방을 걸어 나가면 끊긴다.
         안전한 이유는 라벨이 곧 약속이라는 것뿐이다. */
    const sole = fight.length === 1 ? fight[0] : null;
    items.push(
      sole && sole.id === "attack" && sole.action && !sole.items
        ? { id: "fight", label: "싸우기", action: sole.action }
        : { id: "fight", label: "싸우기", items: fight },
    );
  }
  ...
  /* ★ 설정은 여기 없다. 상태창의 ⚙ 로 **옮겼다**.
     커맨드 창은 '세계에서 지금 할 수 있는 일' 이고 설정은 살림이다 — 색과
     계정은 방에 무엇이 있든 똑같이 있으므로 '상태의 함수' 라는 이 목록의
     성질에서 벗어나 있었다. 그리고 항목 하나가 줄면서 최상위가 네 줄이 된다.
     ★ 복제가 아니라 이동이다. 두 군데에 두면 어느 쪽이 진짜인지 알 수 없고
       (Dpad.tsx 의 규약), 접근성 이름이 같은 버튼이 둘이 되어 검사가 죽는다. */
