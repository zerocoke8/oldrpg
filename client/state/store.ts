...
export function reduce(st: UiState, m: ServerMsg | LocalMsg): UiState {
  switch (m.t) {
    ...
    case "room.describe": {
      ...
      //
      /* ★ 다른 방으로 옮겼으면 로그를 **통째로** 비운다. 로그는 '지금 이 방에서
         무슨 일이 있는가' 를 읽는 창이고, 방을 옮기면 그 전부가 지난 일이 된다.

         처음에는 묘사(narr)만 걷었는데, 그러면 "접수원이 이곳에 있다" 같은
         인물·구조 줄이 다음 방까지 따라와서 없는 사람이 있는 것처럼 읽힌다.
         절반만 지우는 것이 아무것도 안 지우는 것보다 나쁜 자리였다.

         ★ 대가가 있다: 지역 전체에 닿는 외침(yell)도 한 칸 걸으면 사라진다.
           방을 옮기는 것은 사람이 스스로 하는 일이라 '내가 지웠다' 로 읽히고,
           지금은 그쪽이 낫다고 판단했다. 남겨야 할 것이 생기면 kind 로 가른다.
